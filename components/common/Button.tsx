...
const Button: React.FC<ButtonProps> = ({ children, variant = 'primary', size = 'md', isLoading = false, ...props }) => {
  ...
  const baseClasses = "rounded-md font-semibold transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center";
  ...
  const variantClasses = {
    ...
    warning: 'bg-brand-accent hover:bg-yellow-500 text-brand-dark focus:ring-brand-accent',
  };
  ...
  const spinnerColor = variant === 'warning' ? 'text-brand-dark' : 'text-white';
  
  const finalClassName = [baseClasses, variantClasses[variant], sizeClasses[size], className].filter(Boolean).join(' ');
